...
async function getBills(params?: SearchParams): Promise<BillResponse> {
    try {
        ...
        const url = `${baseUrl}/bills?${queryParams}`
    ...
    return (
        <div className="w-full p-5 max-w-7xl mx-auto">
            <div className="mb-6">
                ...
                <p className="text-slate-600 mt-1">Track invoices, usage, and payment status for all customers.</p>
            </div>
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {data.map((bill, index) => (
                                    <tr key={`keyBill-${bill.id}`} className={`${index % 2 === 0 ? "bg-white" : "bg-slate-50"} hover:bg-sky-50 transition-colors`}>
                                        ...
                                        <td className="px-4 py-4 align-top">
                                            <div className="text-sm font-semibold text-slate-900">{bill.customer.name}</div>
                                            <div className="text-xs text-slate-500 mt-1">{bill.customer.customer_number}</div>
                                        </td>
