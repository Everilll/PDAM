...
export default async function ProfilePage() {
    ...
    const safeName = customerProfile.name?.trim() || 'Customer'
    const safePhone = customerProfile.phone?.trim() || '-'
    const safeUsername = customerProfile.user?.username?.trim() || '-'
    const safeAddress = customerProfile.address?.trim() || '-'
    const safeRole = customerProfile.user?.role?.trim() || '-'
                        ...
                        <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 lg:col-span-1">
                            ...
                            <div className="mt-5 space-y-3">
                                <div className="rounded-lg bg-slate-50 p-3">
                                    ...
                                    <p className="mt-1 font-semibold text-slate-800">#{customerProfile.id}</p>
                                </div>
                                <div className="rounded-lg bg-slate-50 p-3">
                                    <p className="text-xs font-medium uppercase tracking-wide text-slate-500">User ID</p>
                                    <p className="mt-1 font-semibold text-slate-800">#{customerProfile.user_id}</p>
                                </div>
                            </div>
                        </section>
                        ...
                        <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 lg:col-span-2">
                            ...
                            <div className="mt-5 grid gap-4 sm:grid-cols-2">
                                ...
                                <div className="rounded-lg border border-slate-200 p-4">
                                    ...
                                    <p className="mt-1 text-slate-900 font-semibold">{safeAddress}</p>
                                </div>
                                <div className="rounded-lg border border-slate-200 p-4">
